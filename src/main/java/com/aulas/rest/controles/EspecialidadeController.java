...
public class EspecialidadeController {
	...
	@PostMapping
	public ResponseEntity<EspecialidadeDTO> salvar(@RequestBody Especialidade especialidade) {
		EspecialidadeDTO user = service.salvar(especialidade);
		return ResponseEntity.status(HttpStatus.CREATED).body(user);
	}

	@PutMapping("/{idespecialidade}")
	public ResponseEntity<EspecialidadeDTO> alterar(@PathVariable("idespecialidade") int idespecialidade, @RequestBody Especialidade especialidade) {
